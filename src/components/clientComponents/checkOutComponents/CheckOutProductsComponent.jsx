...
const CheckOutProductsComponent = ({cartProducts}) => {
    return (
        <div className='space-y-3'>
        {cartProducts?.map(((product, index) => {
            const mrp = product?.product_id?.product_inventory_id?.product_stock[0]?.mrp * product.quantity
            const price = product?.product_id?.product_inventory_id?.product_stock[0]?.price * product.quantity
            const percentage = Math.floor(((mrp - price)/mrp) * 100)
            return (
            <div key={index} className="rounded-lg shadow-md w-full flex">
                    ...
                    <div className="w-36 min-w-36">
                        ...
                        <div className="mt-1 mb-2">
                            <span className="justify-center flex gap-2">
                                <span className=" px-2 rounded-lg bg-gray-300">{product?.quantity} </span><span>Products</span>
                            </span> 
                        </div>
                    </div>
                    <div className="px-2 pt-4 w-full h-full flex-col">
                        <div className="h-5 font-medium font-[arial] text-gray-500 text-sm">{product?.product_id?.product_brand?.Brand_name}</div>
