...
const OrderStatusComponent = ({order}) => {
    ...
    const date = (date)=>{
        ...
        return `${format(new Date(date) , "dd-MM-yyyy h:m aaa")}`
    }
    ...
    const cancelOrderStatus = async()=>{
        ...
        try {
            setLoading(true)
            const res = await axios.patch(`${import.meta.env.VITE_BACKEND_URL}order/cancel/${id}`)
    ...
    return (
        <>
            ...
            <div className='hero gap-2'>
                <button onClick={()=>cancelOrderStatus()} disabled={loading || delivered || canceled} className='btn btn-error hero text-white'>Cancel</button>
            </div>
        </>
    )
}
